...
if ($search !== '') {
    $where_clauses[] = "(
        h.nama_umkm LIKE :search
        OR h.jenis_usaha LIKE :search
        OR h.alamat LIKE :search
...
$total_sql = "
    ...
    FROM umkm_history h
    LEFT JOIN user AS pemilik ON h.id_user = pemilik.id_user
...
<head>
    ...
    <title>Riwayat Perubahan UMKM - Admin</title>
            ...
            <div class="content">
                <div class="card-dashboard">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center align-items-start mb-4 gap-3">
                        <div>
                            <h2>Riwayat Perubahan UMKM</h2>
                            <p>Log riwayat perubahan data profil pelaku UMKM</p>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle">
                            ...
                            <tbody>
                                <?php if (empty($data)): ?>
                                    <tr>
                                        <td colspan="10" class="text-center py-4 text-muted">Tidak ada data riwayat UMKM.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($data as $row): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php $status_umkm = $row['status'] ?? ''; ?>
                                                <span class="<?= ($status_umkm === 'aktif') ? 'disetujui' : (($status_umkm === 'nonaktif') ? 'ditolak' : 'pending') ?>">
                                                    <?= $status_umkm ? ucfirst($status_umkm) : '-'; ?>
                                                </span>
                                            </td>
                                            <td>
                                                <?php $action_type_umkm = $row['action_type'] ?? 'INSERT'; ?>
                                                <span class="badge bg-<?= $action_type_umkm === 'SOFT_DELETE' ? 'danger' : ($action_type_umkm === 'UPDATE' ? 'warning' : 'info') ?>">
                                                    <?= htmlspecialchars($action_type_umkm); ?>
                                                </span>
                                            </td>
